...
const ChatBody = React.memo(React.forwardRef<HTMLDivElement, ChatBodyProps>(
    ({ messages, conversationId, currentUserId, onScroll, isLoadingMore, otherUserName, otherUserImage, isGroupChat, onUpdateMessage, onDeleteMessage }, ref) => {
        ...
        useLayoutEffect(() => {
            if (ref && "current" in ref && ref.current) {
                ...
                if (
                    ...
                    (hasNewMessage && messages[0].senderId === currentUserId)
                ) {
        ...
        const fetchAsset = useCallback(async (messageId: string) => {
            ...
            try {
                const data = await messageUserService.getMessageFile(
                    null,
                    null,
                    messageId,
                );
                ...
                setAssetCache((prev) => ({
                    ...prev,
                    [messageId]: { url, type: data.contentType },
                }));
            } catch (error: any) {
        ...
        useEffect(() => {
            const pendingAssets = messages.filter(
                (m) =>
                    (m.type === "IMG" ||
                        m.type === "VID" ||
                        m.type === "GENERIC_FILE") &&
        ...
        const fetchSenderName = useCallback(
            async (senderId: string) => {
                if (
                    fetchingSenderIds.current.has(senderId) ||
                ...
                try {
                    ...
                    if (conversationId) {
                        try {
                            ...
                            console.error(
                                `Failed to load chat member info for ${senderId}:`,
                                error,
                            );
                        }
                    }
                    ...
                    console.error(
                        `Failed to load sender profile ${senderId}:`,
                        error,
                    );
                } finally {
        ...
        useEffect(() => {
            ...
            const pendingSenders = messages.filter(
                (m) => m.senderId !== currentUserId && !senderCache[m.senderId],
            );
